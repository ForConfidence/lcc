...
<head>
	...
	<link rel="stylesheet" type="text/css"
		...
		<script src="<%=basePath%>js/lcc/pdm_modulebuilder.js" type="text/javascript"></script>
	...
	<script type="text/javascript">
		...
		function main(container, outline, toolbar, modelbar,sidebar, status)
		{
				...
				if(type!=moduleobject.style&&type!='taskprocess'){
					return null;
				}
							 ...
							 winprocess.show('center', 'middle', true);
						   	...
						   	var tasktreeTable = new Edo.data.DataTree()
								...
								function roleSelect()
								   ...
								   if(r!=null){
								   var data= cims201.utils.getData('tasktree/tasktree!getTaskTreebyRole.action',{role:r.id});
			                	...
			                	var box=Edo.create(
									{type: 'box',width: '100%',height:'70%',border: [0,0,0,0],padding: [0,0,0,0],layout: 'vertical',
										...
										var url='department/department!getOperationRoles.action';
										var param={};
										var id='role';
				  ...
				  /*   getPDMmoduledefine(isdefined);
					...
					if(cells.length>0&&!moduleobject.issaved){
						...
						if(moduleobject.moduletype=='PDM'){
						   ...
						   //var moduleuuid= cims201.utils.getData('module/module!addnewmodule.action',{modulename:moduleobject.modulename,modulenote:moduleobject.modulenote,moduletype:moduleobject.moduletype});     
						   var data=null;
						   Edo.util.Ajax.request({
								url : 'module/module!addnewmodule.action',
								type : 'post',
						   ...
						   alert(j)
						       ...
						       for(var i=0;i<modules.length;i++){
					       	   ...
					       	   var data= cims201.utils.getData('module/module!addlevelmodule.action',{levelid:levelid,parentlevelid:parentlevelid,moduleuuid:moduleuuid,cellcollection:cellcollection,xmldata:xmldata,moduletype:moduleobject.moduletype}); 
					       	   cellcollection=new Array;
					       	 }
							/* var enc = new mxCodec(mxUtils.createXmlDocument());
							var node = enc.encode(graph.getModel());
							var xmldata = mxUtils.getPrettyXml(node);
							var data= cims201.utils.getData('module/module!addmodule.action',{modulename:moduleobject.modulename,modulenote:moduleobject.modulenote,cellcollection:cellcollection,xmldata:xmldata,moduletype:moduleobject.moduletype}); */
							/* var xmldata = level.xml;
					        var data= cims201.utils.getData('module/module!addmodule.action',{modulename:moduleobject.modulename,modulenote:moduleobject.modulenote,cellcollection:cellcollection,xmldata:xmldata,moduletype:moduleobject.moduletype});     
							...
							var node = enc.encode(graph.getModel());
							var xmldata = mxUtils.getPrettyXml(node);
							/* var data= cims201.utils.getData('lcc/lccmodule!addmodule.action',{version:moduleobject.versionid,components:moduleobject.components,pdid:moduleobject.productid,modulename:moduleobject.modulename,modulenote:moduleobject.modulenote,cellcollection:cellcollection,xmldata:xmldata,moduletype:moduleobject.moduletype});
		   ...
		   // var data= cims201.utils.getData('node/node!getnodelist.action',{});
	...
	/* 	document.oncontextmenu = function(){
		...
		function getcollection(graph,sidebar,nodetype,nodecategory){
			 var datas= cims201.utils.getData('node/node!getNodeListByType.action',{nodetype:nodetype,nodecategory:nodecategory});
		...
		function configureStylesheet(graph)
		{
			...
			graph.getStylesheet().putCellStyle('newprocess', style);
			style = mxUtils.clone(style);
			graph.getStylesheet().putCellStyle('taskprocess', style);
			tyle = mxUtils.clone(style);
			style[mxConstants.STYLE_FILLCOLOR] = '#3366FF';
			style[mxConstants.STYLE_LABEL_BACKGROUNDCOLOR] = '#3366FF';
			graph.getStylesheet().putCellStyle('oldprocess', style);
		...
		//initgraph();
		var v011 = addnewprocess('市场分析及确定需求',100, 100, 180, 100,'newprocess','#00CCFF');
		var v012 =addnewprocess('确定功能原理和功能模块及系列化规划',100, 250, 180, 100,'newprocess','#00CCFF'); 
		var v013 =  addnewprocess('寻找原理方案及总体结构规划',100, 400,180, 100,'newprocess','#00CCFF');
		var v014 = addnewprocess('结构模块划分',100, 550, 180, 100,'newprocess','#00CCFF');
		...
		var v021 = addnewprocess('阐明任务确定需求',400, 100,180, 100,'oldprocess','#3366FF');
		var v022 =addnewprocess('零部件ABC分析',400, 250, 180, 100,'oldprocess','#3366FF'); 
		var v023 =  addnewprocess('产品功能系列化和模块化规划',400, 400, 180, 100,'oldprocess','#3366FF');
		var v024 = addnewprocess('结构模块优化',400, 550, 180, 100,'oldprocess','#3366FF');
		...
		var v05 = addnewprocess('建立产品编码和名称字典',250, 700, 180, 100,'oldprocess','#3366FF');
		var v06 = addnewprocess('标准模块设计和模块接口定义',250, 850,180, 100,'oldprocess','#3366FF');
		var v07 =addnewprocess('模块化产品及平台总体构建',250, 1000, 180, 100,'oldprocess','#3366FF'); 
		var v08 =  addnewprocess('产品结构及系列化设计',250, 1150, 180, 100,'oldprocess','#3366FF');
		var v09 =  addnewprocess('产品模块化平台',250, 1300, 180, 100,'oldprocess','#3366FF');
		var v10 = addnewprocess('产品配置设计',250, 1450,180, 100,'oldprocess','#3366FF');
		var v11 =addnewprocess('产品变型设计',250, 1600,180, 100,'oldprocess','#3366FF'); 
	   ...
	   /*  var levelmodulev11=wholelevel.addlevelmodule(v11);
		...
		function buildnewproductpdmmodule(){
		var graph=editor.graph;
		var parent = editor.graph.getDefaultParent();
		var v011 = addnewprocess('市场分析及确定需求',100, 100, 180, 100,'newprocess','#00CCFF');
		var v012 =addnewprocess('确定功能原理和功能模块及系列化规划',100, 250, 180, 100,'newprocess','#00CCFF'); 
		...
		var v05 = addnewprocess('建立产品编码和名称字典',250, 700, 180, 100,'newprocess','#00CCFF');
		var v06 = addnewprocess('标准模块设计和模块接口定义',250, 850,180, 100,'newprocess','#00CCFF');
		var v07 =addnewprocess('模块化产品及平台总体构建',250, 1000, 180, 100,'newprocess','#00CCFF'); 
		var v08 =  addnewprocess('产品结构及系列化设计',250, 1150, 180, 100,'newprocess','#00CCFF');
		var v09 =  addnewprocess('产品模块化平台',250, 1300, 180, 100,'newprocess','#00CCFF');
		var v10 = addnewprocess('产品配置设计',250, 1450,180, 100,'newprocess','#00CCFF');
		var v11 =addnewprocess('产品变型设计',250, 1600,180, 100,'newprocess','#00CCFF'); 
