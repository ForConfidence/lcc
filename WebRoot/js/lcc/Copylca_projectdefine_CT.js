...
function showdetail(editcell,module,moduleid){
	//alert(b.offsetWidth)
		...
		var data= cims201.utils.getData('lcc/lccmodule!getmoduleprocesscontent.action',{moduleid:currentmoduleid,processid:cell.getId()});
...
function getmodulebox(){
	var moduledata= cims201.utils.getData('lcc/lccmodule!getModuletree.action',{componentid:projectobj.productid});
	var componentmodeltree=getcomponentmodeltree();
	var moduledetail=getmoduledetail();
	var modulebox=Edo.create(
			{type: 'ct',id:'modulebox',width: 600,height: 600,padding:[0,0,0,0],border:[0,0,0,0],layout:'horizontal',horizontalGap:'0',
	              children:[
	                       {id: 'leftPanel',type: 'panel',title: '模板浏览',width: 300,height:600,padding:[0,0,0,0],verticalGap:'0',
	                    	   border:[0,0,0,0],
	                    	   children: [
                	              {
									type: 'group',
								    ...
								    children: [
								        {
								            ...
								            onclick: function(e){
								            	...
								            	projectobj.module.moduleid=r.id;
								            	projectobj.module.modulename=r.name;
									        	projectobj.module.Createdate=r.Createdate;
									        	projectobj.module.Version=r.Version;
									        	projectobj.module.createuserid=r.createuserid;
									        	Edo.util.Ajax.request({
							       			        type: 'post',        
							       			        url: 'lcc/lccmodule!addsolution.action',
							       			        params: {
							       		                projectname:projectobj.projectname, 
							       		                projectnote:projectobj.projectnote,
							       		                pdid:projectobj.productid,
							       		                moduleid:projectobj.module.moduleid
							       		            },
							       			        onSuccess: function(text){
							       			            var data = Edo.util.Json.decode(text);
							       			            projectobj.projectid=data;
							       			            alert("创建成功！");
							       			            aa.deliverprojectdid(projectobj);
							       			         modulebox.destroy();
	             ...
	             {type: 'panel',title: '模板零部件树',width: 300,height: '100%',padding:[0,0,0,0],verticalGap:'0', border:[0,0,0,0],
	                    children: [
							{
								type: 'group',
								width: '100%',
