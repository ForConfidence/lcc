	...
	function startCalculator(cId){
		...
		Edo.create({
			...
			children: [{
				...
				children: [
					{type: 'button',width: 120, text: '设计阶段', icon: 'e-icon-design'},
					{type: 'button',width: 150, text: '原材料获取阶段', icon: 'e-icon-material'},
					{type: 'button',width: 150, text: '制造阶段', icon: 'e-icon-manufacture'},
					{type: 'button',width: 150, text: '运输销售阶段', icon: 'e-icon-transport'},
					{type: 'button',width: 150, text: '服装使用阶段', icon: 'e-icon-service'},
					{type: 'button',width: 130, text: '回收处理阶段', icon: 'e-icon-recycle'}
				]
			},
			{
				id: 'tabContent',
	...
	var dataPanel = Edo.create({
		...
		children: [
			{type: 'box', width: '100%', border: 0, padding: 0, layout: 'horizontal', children: [
				{type: 'label', text: '设计阶段:', width: 200},
				{type: 'label', text: '0', id: 'stageresult_1'},
				{type: 'label', text: 'kg'}
			]},
			
			{type: 'box', width: '100%', border: 0, padding: 0, layout: 'horizontal', children: [
			 	{type: 'label', text: '原材料获取阶段:', width: 200},
			 	{type: 'label', text: '0', id: 'stageresult_2'},
			 	{type: 'label', text: 'kg'}
			]},
			
			{type: 'box', width: '100%', border: 0, padding: 0, layout: 'horizontal', children: [
				{type: 'label', text: '制造阶段:', width: 200},
				{type: 'label', text: '0', id: 'stageresult_3'},
				{type: 'label', text: 'kg'}
			]},
			{type: 'box', width: '100%', border: 0, padding: 0, layout: 'horizontal', children: [
				{type: 'label', text: '运输和销售阶段:', width: 200},
				{type: 'label', text: '0', id: 'stageresult_4'},
				{type: 'label', text: 'kg'}
			]},
			{type: 'box', width: '100%', border: 0, padding: 0, layout: 'horizontal', children: [
				{type: 'label', text: '服务阶段:', width: 200},
				{type: 'label', text: '0', id: 'stageresult_5'},
				{type: 'label', text: 'kg'}
			]},
			{type: 'box', width: '100%', border: 0, padding: 0, layout: 'horizontal', children: [
				{type: 'label', text: '回收阶段:', width: 200},
				{type: 'label', text: '0', id: 'stageresult_6'},
				{type: 'label', text: 'kg'}
			]},
			{type: 'box', width: '100%', border: 0, padding: 0, layout: 'horizontal', children: [
				{type: 'label', text: '计算结果:', width: 200},
				{type: 'label', text: '0', id: 'finalresult'},
				{type: 'label', text: 'kg'}
			]},
			{type: 'box', width: '100%', border: 0, padding: 0, layout: 'horizontal', children: [
				{type: 'button', width: '100', text: '计算', onclick: function(e){
				...
				{type: 'button', width: '100', text: '重置', onclick: function(e){
					resultjsonarray = "";
					for(var i=1; i<=5; i++){							
				...
				{type: 'button', width: '100', text: '保存', onclick: function(){
					var or = new Array();
					for(var i=1; i<=5; i++){							
						Edo.get('calTable_'+i).data.source.each(function(o){							
							or[or.length] = {
					...
					Edo.util.Ajax.request({
				        type: 'post',        
				        url: 'lcc/calculator!saveCarbonEmission.action',
				        params: {
	    	...
	    	children: [
				{
					...
					height: '100%',
			   		border: [0,0,0,0],
			   		src: 'lcc/calculator!showChart.action?data=' + data
				}
			]
		});
		outPanel = Edo.create({
	...
	function createStageCalculator(stage, cId){	    
	    ...
	    var calTable = Edo.create({
        	...
        	//	Edo.lists.Table.createMultiColumn(), 		
                ...
                {header: '原材料', width: 180,  dataIndex: 'name', headerAlign: 'center', align: 'center'},        
                {header: 'CO<font size =1px>2</font> 排放/每单位(kg)',width: 200, dataIndex: 'emission', headerAlign: 'center', align: 'center', editor: {
                	type: 'text'
                },valid: function(v,r){
                ...
                {header: '使用总量(unit)', dataIndex: 'sum', width: 180, headerAlign: 'center', align: 'center', editor: {
                	type: 'text'
                },valid: function(v,r){
                	var reg = /^(([0-9]+\.[0-9]*[1-9][0-9]*)|([0-9]*[1-9][0-9]*\.[0-9]+)|([0-9]*[1-9][0-9]*))$/;             	
                	if(reg.exec(v)){               		
                ...
                {header: '描述', dataIndex: 'description', width: 450, headerAlign: 'center'}                
        	],
        	onbodymousedown: function(e){
        		...
        		if('emission' == e.column.dataIndex){
        			Edo.util.Ajax.request({
				        type: 'post',        
				        url: 'lcc/calculator!carbonEmissionStatistics.action',
				        params: {
		...
		Edo.util.Ajax.request({
	        type: 'post',        
	        url: 'lcc/calculator!getMaterialByStage.action',
	        params: {
			        		...
			        		if(rs.length <= 0){
			        			...
			        			Edo.MessageBox.confirm("confirm", "you want to delete material "+rs[0].name+'?', function(result){
									if('yes' == result){
										...
										Edo.util.Ajax.request({
									        type: 'post',        
									        url: 'lcc/calculator!deleteMaterial.action',
									        params: {
			    ...
			    children: [
			    	...
			    	},
			    	{
			    		...
			    		onclick: function(e){
			    			...
			    			Edo.util.Ajax.request({
								url: 'lcc/calculator!addMaterial.action',
								type : 'post',
								params : o,
								async : false,
								onSuccess : function(text) {
			    ...
			    children: [
			    	...
			    	},
			    	{
			    		...
			    		onclick: function(e){
			    			...
			    			Edo.util.Ajax.request({
								url: 'lcc/calculator!updateMaterial.action',
								type : 'post',
								params : m,
								async : false,
								onSuccess : function(text) {
