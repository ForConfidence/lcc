...
*/	
	...
	var myIndexTree = cims201.tree.createIndexTree();
	
	
		
	//创建按钮
	var methodNames = ['添加节点','删除','编辑'];
	
	var methodFunctions = [
		function(e){
			currentNode = myIndexTree.currentNode;
			if(currentNode != null){
				if(myForm == null){
					myForm = cims201.tree.createTreeNodeForm();
				}
				if(myWin == null) 
					myWin = cims201.utils.getWin(420,400,'',myForm);
				
				Edo.get('indexSubmitBtn1').clearEvent();
				Edo.get('indexSubmitBtn1').set('onclick',function(){
					
						var o = myForm.getForm();
						o.id = currentNode.id;
						var newNode = cims201.utils.getData('product/mdeindex!saveNode.action',o);
						cims201.tree.addTreeNode(myIndexTree,newNode);
						Edo.MessageBox.confirm("保存成功", "保存成功！", null);
						myWin.hide();
				});
			
					
				myWin.set('title','Add Node');
				cims201.tree.addIndexTreeNode(myForm);
				setWinScreenPosition(420,400,myWin,null);
				//myWin.show(100,250,true);	
			}else{
				cims201.utils.warn('提醒',Edo.MessageBox.WARNING,'请选择一个节点！',Edo.MessageBox.YESNOCANCEL); 
			}	
		},
		...
		function(e){
			...
			if(currentNode != null){
				...
				Edo.MessageBox.confirm("提醒", "确定删除节点"+currentNode.nodeName, function(action){
				if(action=='yes'){
					var result=cims201.utils.getData('product/mdeindex!deleteIndex.action',currentNode);
					if(result.indexOf("该节点不能删除:")!=-1){
						alert(result);
			
					}else if('error' !=result ){
						Edo.MessageBox.confirm("消息", "删除成功！", null);
						cims201.tree.deleteTreeNode(myIndexTree);		
		...
		function(e){
			var currentNode = myIndexTree.currentNode;
			if(currentNode != null){
				if(myForm == null){
					myForm = cims201.tree.createTreeNodeForm();
				}
				...
				Edo.get('indexSubmitBtn1').set('onclick',function(){
					var o = myForm.getForm();
					o.id=currentNode.id;
					var newNode = cims201.utils.getData('product/mdeindex!updateIndex.action',o);
					
					cims201.tree.editTreeNode(myIndexTree,newNode);
					Edo.MessageBox.confirm("消息", "修改成功！", null);
					myWin.hide();
					
				});
				
				myWin.set('title','编辑指标');
				var r = myIndexTree.currentNode;
				cims201.tree.editIndexTreeNode(myForm,r);	
			//	myWin.show(200,200,true);	
				setWinScreenPosition(420,400,myWin,null);
				
			}else{
				cims201.utils.warn('提醒',Edo.MessageBox.WARNING,'必须选择一个节点',Edo.MessageBox.YESNOCANCEL); 
			}
			}
	...
	function addTreeNode(e){
			...
			var isb = Edo.get('indexSubmitBtn1');
			isb.clearEvent();
			isb.set('onclick',function(e){
				var o = myForm.getForm();
				o.json="indexTree";
				var newNode = cims201.utils.getData('product/mdeindex!saveNode.action',o);
				cims201.tree.addTopTreeNode(myIndexTree,newNode);	
				Edo.MessageBox.confirm("消息", "保存成功！", null);
				
				myWin.hide();
			});
		
				
			myWin.set('title','添加指标');
			cims201.tree.addIndexTreeNode(myForm);
			setWinScreenPosition(420,400,myWin,null);
			//myWin.show(200,200,true);	
		}
	...
	var btPanel = cims201.utils.createBtPanel(methodNames,methodFunctions);
