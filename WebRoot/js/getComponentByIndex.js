...
//			});
	...
	else if('mytask'==index){
		 var existcomponent = iscomponentexist(id,'mytask234');
		if(null!=existcomponent){
			myComponent=existcomponent;
		}else{
			var task= new getmytask();
			enterEventList[enterEventList.length] = task;
			enterEventIDList[enterEventIDList.length] = 'mytask';
	...
	else if('TemplateCreate'==index){
		var existcomponent = iscomponentexist(id,'TemplateCreate234');
		if(null!=existcomponent){
			myComponent=existcomponent;
		}else{
			var myTemplateCreate = new createTemplateCreate();
			enterEventList[enterEventList.length] = myTemplateCreate;
			enterEventIDList[enterEventIDList.length] = 'TemplateCreate';
		
			myComponent = Edo.create({
				id: id?id:'TemplateCreate234',   
			    type: 'box',                 
			    //layout: 'vertical',  
			    //verticalGap:0,
			    width: '100%',
		...
		var existcomponent = iscomponentexist(id,'TemplateManage234');

		if(null!=existcomponent){
			myComponent=existcomponent;
		}else{
			//父件销毁，有id的子键仍然存在？
			if(Edo.get('templateDetailT')){
				Edo.get('templateDetailT').destroy ( );
			}
			if(Edo.get('templateDetailTextarea')){
				Edo.get('templateDetailTextarea').destroy ( );
			}
			...
			myComponent = Edo.create({
			    ...
			    padding: [0,0,0,0],
			    children: [
					myTemplateManage.getTemplateDetail(),
					myTemplateManage.getTemplateLoad()
			    ]
			});
		}
	}
