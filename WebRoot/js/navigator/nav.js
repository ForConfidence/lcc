...
var updata = [
        ...
        {id: '5', index: 'oldknowledgeupload', type:'knowledgeupload',name: '上传通道2'},
        {id: '10', index: 'dandianknowledgeupload', type:'knowledgeupload',name: '单点上传'},
...
function navManager(type,filename){
	...
	if('user' == type){		
   ...
   }else if('mde' == type){// 江丁丁添加  2013-1-15 模块化设计评价MDE 
		if(myNavPanel){
			myNavPanel.destroy();
		}
		myNavPanel = Edo.create({
		...
		openNewTab('cims201_mdehome', 'mdehome', '<div class=cims201_tab_font align=center>模块化评价</div>', {btIcon:'cims201_mde_icon_small'});
		leftRegion.set('visible',false);
	}else if('lca' == type){//杨文君 2013-8-13添加   全生命周期评价LCA
		if(myNavPanel){
			myNavPanel.destroy();
		}
		myNavPanel = Edo.create({
		...
		openNewTab('cims201_lcahome', 'lcahome', '<div class=cims201_tab_font align=center>全生命周期评价</div>', {btIcon:'cims201_lca_icon_small'});
		leftRegion.set('visible',false);
...
//			myNavPanel.destroy();
//		}
//		openNewTab('cims201_kmap', 'kmap', '<div class=cims201_tab_font align=center>知识地图</div>', {});
//		leftRegion.set('visible',false);
		if(myNavPanel){
			myNavPanel.destroy();
		}
...
//		leftRegion.set('visible',false);
		...
		if(Edo.get('mapNav')){
			myNavPanel.addChild(Edo.get('mapNav'));
		}else{
			myNavPanel.addChild(new createNav_map().getMap());
		}
		leftRegion.addChild(myNavPanel);
	...
	if(myNavPanel){
		...
		myNavPanel = Edo.create({
			id:'leftPanel',
			type:'panel',
			title:'<div class = cims201_tab_font align=center>编码管理</div>',
			width:'100%',
			height:'100%',
			padding:[0,0,0,0]
		});
		if(Edo.get('codeclassNav')){
			myNavPanel.addChild(Edo.get('codeclassNav'));
		}else{
			myNavPanel.addChild(new createNav_codeclass().getCodeclass());
		}
		leftRegion.addChild(myNavPanel);
...
//	                        {id: 23, index: 'patenteuropeansearch', name: '欧洲专利'},
//	                        {id: 24, index: 'patentjapansearch', name: '日本专利'},
//	                        {id: 25, index: 'patentlocalsearch', name: '本地专利'}
	                    ]
	                }
	            ]
	        }
	            ...
	            children: [
	                {type: 'table', autoColumns: true, headerVisible: false,width: '100%', height: '100%',style:'border:0;',verticalLine: false, horizontalLine: false, //rowHeight: 25,
	                    ...
	                    data: [
	                        ...
	                        //{id: 35, index: 'expertweihu', name: '&nbsp;&nbsp;&nbsp;专家<br>信息维护', showname: '信息维护'},
	                        //{id: 36, index: 'expertdaohang', name: '&nbsp专家分类'+'<br>'+'导航与查询', showname: '专家导航'},
	                       // {id: 36, index: 'expertdaohang', name: '领域专家'+'<br>'+'问题导航', showname: '专家导航'},
	                       // {id: 37, index: 'expertjieda', name: '专家解答', showname: '专家解答'},
	                        //{id: 37, index: 'expertjieda', name: '设置专家', showname: '设置专家'},
	                        //{id: 40, index: 'experttuijian', name: '专家推荐', showname: '专家推荐'} 
...
/*function createNav_codeclass(){
	var myCodeClassNav=Edo.create({
		id:'codeclassNav',
		type: 'tree', autoColumns: true, headerVisible: false,width: '100%', height: '100%',style:'border:0;',verticalLine: false, horizontalLine: false, //rowHeight: 25,
        onselectionchange: function(e){	
        	if(e.selected.index){
        	 	openNewTab(e.selected.id, e.selected.index,"<div class=cims201_tab_font align=center>"+e.selected.name+"</div>",{type:e.selected.index,btIcon:'cims201_myknowledgebase_icon_'+e.selected.index+'_small'});
        	}
        ...
        data: [
        	{name:'编码规则管理' ,children: [
        		{id:001,index:'codeclassdefi',name:'零部件分类定义'},
        		{id:002,index:'CodeClassRuleManage',name:'分类编码规则管理'}
        	]},
        	{name:'编码分类结构管理' ,children: [
        		{id:003,index:'CodeClassManage',name:'编码大类管理'},
        		{id:004,index:'CodeClassStrutsManage',name:'编码大类结构管理'},
        		{id:005,index:'CodeClassEdit',name:'分类编码编辑'},
        		{id:006,index:'CodeClassCustom',name:'分类结构定制'}
        	]},
        	
        	{id:007,index:'ShuXingManage',name:'属性管理'},
        	{id:008,index:'InstanceEdit',name:'实例编辑'}
        ]
    
	});
	this.getCodeclass=function(){
		return myCodeClassNav;
	};
}*/
