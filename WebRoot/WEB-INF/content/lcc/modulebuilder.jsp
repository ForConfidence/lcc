...
<head>
	...
	<link rel="stylesheet" type="text/css"
		...
		<script src="<%=basePath%>js/lcc/lca_modulebuilder_builder.js" type="text/javascript"></script>
	 ...
	 /*    var tr=popmenu.addItem ('删除', 'images/delete.gif', deletefunc, popmenu);
		...
		function main(container, outline, toolbar, modelbar,sidebar, status)
		{
				...
				{ 
					...
					if(moduleobject.buildtype=='new_coopbuild'||(moduleobject.buildtype=='old_coopbuild'&&moduleobject.oldbuildtype=='new')){
							...
							for(var j=0;j<moduleobject.componentstageslist.length;j++)
							{
								...
								{
								if(!moduleobject.componentstageslist[j].stages[k].isassigned)
									{
									 iscompleted=false;
									 break;
									}
						    ...
						    if(iscompleted){
							    if(moduleobject.buildtype=='new_coopbuild'){
							    if(moduleobject.supermoduleid==null){
								    var func=function(){
						             		moduleobject.modulename=Edo.get('mdname').text;
						             		moduleobject.modulenote=Edo.get('mdnote').text;
						             		cellcollection=moduleobject.componentstageslist;
						             		var enc = new mxCodec(mxUtils.createXmlDocument());
											var node = enc.encode(graph.getModel());
											var xmldata = mxUtils.getPrettyXml(node);
											var data= cims201.utils.getData(basePath+'lcc/lccmodule!addCoopModule.action',{superisassigned:moduleobject.superisassigned,superparentmoduleid:moduleobject.supermoduleid,branchUUID:moduleobject.branchUUID,pdid:moduleobject.productid,modulename:moduleobject.modulename,modulenote:moduleobject.modulenote,cellcollection:cellcollection,xmldata:xmldata,buildtype:moduleobject.buildtype});
											if(data.needassign==1){
												moduleobject.branchUUID=data.branchUUID;
												moduleobject.supermoduleid=data.supermoduleid;
											    moduleobject.superisassigned=1;
						             	...
						             	var content = Edo.create(
										    {type: 'box',width: '100%',height:'70%',border: [0,0,0,0],padding: [0,0,0,0],layout: 'vertical',
									       	    children: [
										       	    {type : 'formitem',label : '模板名称:',width: '100%',labelWidth : 150,labelAlign : 'right',
										       	    children : [{type : 'text',width : 200,id : 'mdname'}]
										       	    },
										       	    {type : 'formitem',label : '模板备注:',width: '100%',labelWidth : 150,labelAlign : 'right',
										       	    children : [{type : 'text',width : 200,id : 'mdnote'}]
										       	    },
										       	    {type : 'formitem',label : '产品类别:',width: '100%',labelWidth : 150,labelAlign : 'right',
										       	    children : [{type : 'text',width : 200,id : 'pdname'},{type : 'text',width : 200,visible:false ,id : 'pdid'}]
										       	    } 
										       	    ]
										       	});
									    var toolbar=new gettoolbar(null,func);
									    Edo.get('pdname').set('text',moduleobject.productname);
									    Edo.get('pdid').set('text', moduleobject.productid);
							    ...
							    //已有模板协同创建，目前功能上用处不大，没有写完.
							    	var oldsupermoduleid=moduleobject.oldsupermoduleid;
							   		cellcollection=moduleobject.componentstageslist;
				             		var enc = new mxCodec(mxUtils.createXmlDocument());
									var node = enc.encode(graph.getModel());
									var xmldata = mxUtils.getPrettyXml(node);
									alert(oldsupermoduleid)
									var data= cims201.utils.getData(basePath+'lcc/lccmodule!addOldModule.action',{oldsupermoduleid:oldsupermoduleid,cellcollection:cellcollection,xmldata:xmldata,buildtype:moduleobject.buildtype});
									moduleobject.issaved=true;
									parent.savecompleted();
									alert(data);
								  	alert('保存成功！');
					...
					}else if(moduleobject.buildtype=='old_coopbuild'&&moduleobject.oldbuildtype=='old'){
							savecells();
						    alert('老的协同')
					     ...
					     var data= cims201.utils.getData(basePath+'lcc/lccmodule!simpleModuleSave.action',
					     {superisassigned:moduleobject.superisassigned,superparentmoduleid:moduleobject.supermoduleid,branchUUID:moduleobject.branchUUID,alldata:alldata});
						 return;
					...
					}else if(moduleobject.buildtype=='new_simplebuild'||moduleobject.buildtype=='old_simplebuild'){
						if(moduleobject.oldbuildtype=='old'){
						   savecells();
						   alert('老的保存')
							var alllevels=wholelevel.alllevels;
							var alldata=moduleobject.oldmodulealldata;
					        ...
					        alldata.oldbuildtype=moduleobject.oldbuildtype;
					        alldata.oldsupermoduleid=moduleobject.oldsupermoduleid;
					        alldata.oldbranchUUID=moduleobject.oldbranchUUID;
					        alldata.moduleid=moduleobject.oldmoduleid;
					      	for(var j=0;j<alllevels.length;j++){
					      	    ...
					      	    cellcollection=new Array();
					      	 	...
					      	 	//传递原始模板的顶级id
					       var data= cims201.utils.getData(basePath+'lcc/lccmodule!simpleModuleSave.action',
					        {superisassigned:moduleobject.superisassigned,branchUUID:moduleobject.branchUUID,alldata:alldata});
				           if(data.needassign==1){
								alert('已完成第一个，赋值');
								moduleobject.branchUUID=data.branchUUID;
							    moduleobject.superisassigned=1;
					 ...
					 */       if(moduleobject.supermoduleid==null){
									var func=function(){
								        ...
								        var alllevels=wholelevel.alllevels;
								      	...
								      	var text=Edo.util.JSON.encode(alldata);
								       if(moduleobject.oldbuildtype=='new'){
									       alldata.oldsupermoduleid=moduleobject.oldsupermoduleid;
									       alldata.oldbranchUUID=moduleobject.oldbranchUUID;
									       alldata.moduleid=moduleobject.oldmoduleid;
								       }
								        var data= cims201.utils.getData(basePath+'lcc/lccmodule!simpleModuleSave.action',{superisassigned:moduleobject.superisassigned,superparentmoduleid:moduleobject.supermoduleid,branchUUID:moduleobject.branchUUID,alldata:alldata});
										alert(data.needassign);
										if(data.needassign==1){
											moduleobject.branchUUID=data.branchUUID;
											moduleobject.supermoduleid=data.supermoduleid;
										    moduleobject.superisassigned=1;
								...
								alert(alllevels.length);
							       ...
							       for(var i=0;i<modules.length;i++){
								       ...
								       alert('是process')
								            var cell={};
								      		cell.processname=modules[i].levelmoduleobject.processname;
								      		alert('过程名称'+modules[i].levelmoduleobject.processname);
								        	cell.processnote=modules[i].levelmoduleobject.processnote;
								            cell.id=modules[i].levelmoduleobject.cellid;
								            cell.knowledge=modules[i].levelmoduleobject.knowledge;
								            cell.inputmaterial=modules[i].levelmoduleobject.inputmaterial;
						       	  ...
						       	  /*  if('level_0'==levelid){
						       	    	var data= cims201.utils.getData(basePath+'lcc/lccmodule!addlevelmodule.action',{levelid:levelid,parentlevelid:parentlevelid,taskid:moduleobject.taskid,cellcollection:cellcollection,xmldata:xmldata}); 
						       	   		
						       	   }else{
						       	    	var parentcellid=level.parentcellid;
						       	   		var data= cims201.utils.getData(basePath+'lcc/lccmodule!addlevelmodule.action',{levelid:levelid,parentcellid:parentcellid,taskid:moduleobject.taskid,parentlevelid:parentlevelid,cellcollection:cellcollection,xmldata:xmldata});
						       	   } */
								//}
	...
	/* 	document.oncontextmenu = function(){
		...
		{  
			   ...
			   if (level.type=='stagelevel'){  
		       	  if (cell != null){
		       	       if (model.isVertex(cell)&&cell.getStyle()=='process'){
			        		...
			        		menu.addItem('指定创建人员', null, function(){  
		   ...
		   }else if(moduleobject.buildtype=='old_coopbuild'){
			   if (level.type=='stagelevel'){  
			       	  if (cell != null){
			       	       if (model.isVertex(cell)&&cell.getStyle()=='process'){
		   ...
		   }else if(moduleobject.buildtype=='old_simplebuild'&& moduleobject.oldbuildtype=='old'){
		   if (cell != null){
				   if (model.isVertex(cell)&&cell.getStyle()=='process'){
														       	 ...
														       	 if(childlevel!=null){
														             ...
														             return;
																       ...
																       Edo.util.Ajax.request({
														                url: basePath+'lcc/lccmodule!getsubmodule.action',
														                params: {
														                	parentmoduleid:parentmoduleid,
														                    processid: processid,
														                    superparentmoduleid:moduleobject.oldmoduleid
		   ...
		   }else if(moduleobject.buildtype=='new_simplebuild'||(moduleobject.buildtype=='old_simplebuild'&& moduleobject.oldbuildtype=='new')){
			   if (cell != null){
				   if (model.isVertex(cell)&&cell.getStyle()=='process'){
						       	        menu.addItem('编辑下一层', null, function()  
								            {  
