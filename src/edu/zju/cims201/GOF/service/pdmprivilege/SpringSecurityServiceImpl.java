package edu.zju.cims201.GOF.service.pdmprivilege;
...
public void findDirectNodes(Long knowledgeID,Set<Long> directCNodeIDS,Set<Long> directDNodeIDS,Connection connection){
	...
	public Set<TripleDTO> getRoleTriples(String operationName,Set<Long> allcdNodeIDS,Set<Long> allrNodeIDS,Connection connection){
		...
		try {
			...
			if(operationName!=null&&allcdNodeIDS!=null&&allrNodeIDS==null){
				...
				inString+=")";	
				sql="SELECT  a.role_tree_nodeid,a.cd_tree_nodeid,a.operation_rightid FROM  CALTKS_ROLE_PRIVILEGE_TRIPLE a, CALTKS_OPERATION_RIGHT right " +
				"where " +
	...
	public Set<Long> listParentNodes(Set<Long> directRoles,Connection connection){
		...
		try {
			...
			inString=inString.substring(0, inString.length()-1);
			inString+=")";
			ps1 = connection.prepareStatement("SELECT  id,PARENT_ID FROM CALTKS_TREE_NODE where  id in "+inString);
			rs1=ps1.executeQuery();
			while(rs1.next()){
				allRoles.add(rs1.getLong(1));
		        getparent(rs1.getLong(2),allRoles,connection);
			}
	...
	public void getparent(Long parentid,Set<Long> allRoles,Connection connection){
		ResultSet rs1=null;
		PreparedStatement ps1=null;
		try {
		ps1 = connection.prepareStatement("SELECT  id,PARENT_ID FROM CALTKS_TREE_NODE where id ="+parentid);
		rs1=ps1.executeQuery();
		while(rs1.next()){
			allRoles.add(rs1.getLong(1));
	        getparent(rs1.getLong(2),allRoles,connection);
	...
	public Set<Long> listSubNodes(Set<Long> directNodes, Connection connection) {
		...
		try {
			...
			inString=inString.substring(0, inString.length()-1);
			inString+=")";
			ps1 = connection.prepareStatement("select id from CALTKS_TREE_NODE where id in"+inString);
			rs1=ps1.executeQuery();
			while(rs1.next()){
				long id=rs1.getLong(1);
				allSubNodes.add(id);
				getchildren(id,allSubNodes,connection);
			}
	...
	public void getchildren(Long id,Set<Long> allSubNodes,Connection connection){
		ResultSet rs1=null;
		PreparedStatement ps1=null;
		try {
		ps1 = connection.prepareStatement("SELECT id FROM CALTKS_TREE_NODE where PARENT_ID ="+id);
		rs1=ps1.executeQuery();
		while(rs1.next()){
			long parentid=rs1.getLong(1);
			allSubNodes.add(parentid);
			System.out.println("树节点树节点树节点树节点树节点树节点树节点树节点树节点树节点树节点树节点"+parentid);
			getchildren(parentid,allSubNodes,connection);
		}
		} catch (SQLException e) {
			e.printStackTrace();
		}finally{
